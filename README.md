...
- [Word fishing](./games/word-fishing/) (library title "Ordfiske") - a whole fishing trip on the water: hook a fish, wind it in tap by tap, read the word it carries and put it in the crate the word belongs to. Every word caught is written into the fishing book; every finished trip decorates the seabed a little more, and catching all forty words (ten in each crate) fills the book. A fish that belongs in another crate simply swims on, nothing is ever timed, no card interrupts the fishing, and no progress is lost - the book, the trip, the stamps and the reef all survive a page reload.
