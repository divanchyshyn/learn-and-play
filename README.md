...
- [Card battle](./games/card-battle/) (library title "Kortkrig", hidden) - solve the number riddle Rex deals: one card from your hand completes the addition or subtraction, and every solved riddle flips over a new animal card for the album. Sixteen animals across four pages, with the maths growing from sums under ten to mixed plus and minus; a riddle solved without a wrong try earns a star, a full page earns a medal, and the finished album celebrates with confetti and an explicit "Start på nytt". Found animals survive a page reload. Still in development, so it is not shown in the game library, but it is built and reachable at [/games/card-battle/](./games/card-battle/).
...
- [Word fishing](./games/word-fishing/) (library title "Ordfiske") - a whole fishing trip on the water, played with one finger on a tablet: tap the water and the boat sails there, lower the bait into the water under the boat, and wait for a fish to take an interest - it swims over, tastes the float a few times, and then the float goes under for a moment or two. Answer in time and the fish is on the line; now the line has to be kept working - hold the crank to wind the fish in, ease off before the arc turns red or the line snaps, and wind again before it goes slack, because a line nobody pulls on lets the fish throw the hook. Read the word it carries and put it in the crate its meaning belongs to - animals, food, nature or home. Reading is the whole task: every word is five letters at most, and nothing in the game is ever read aloud - the caught word is plain text, not a button. Every word caught is written into the fishing book and never swims again, and a fresh fish slips into the water the moment a catch lands, so the sea is never empty. Every trip carries all four crates and ends after four sorted catches, decorating a bigger, livelier seabed a little more - sixteen finds over sixteen trips, from the first starfish to the wreck, the submarine and the underwater station, every find drawn as its own little vector scene (a detailed boat and angler, six fish species, and the treasures themselves) and listed in the fishing book. Catching ten words in each crate - every crate draws from a bigger pool of twenty, so a new run meets other words - fills the book and finishes the game with confetti, a congratulations screen and a "Start på nytt" button. A bite nobody answered, a snapped line and a fish that belongs in another crate all mean the same friendly thing - the fish swims on and the child tries again - no card interrupts the fishing, no miss is ever counted, and the book, the trip, the stamps and the reef all survive a page reload.
